...
function Cart() {
  ...
  return (
    <div className='w-full bg-gray-100 p-4'>
     {
      products.length > 0 ? (
        <div className='container mx-auto h-auto grid grid-cols mdl:grid-cols-2 xl:grid-cols-5 gap-8'>
        <div className='w-full h-full bg-white px-4 col-span-4'>
            <div className='font-titleFont flex items-center justify-between border-b-[1px] border-b-gray-400 py-3'>
              <h2 className='text-2xl md:text-3xl font-medium '>Shopping Cart</h2>
              <h4 className='hidden mdl:inline-flex text-xl font-normal'>Subtotal</h4>
            </div>
            ...
            <div>
                {
                 products.map((item)=>(
                  <div key={item.id} className='w-full border-b-[1px] border-b-gray-300 p-4 flex items-center gap-6'>
                    <div className='w-full flex mdl:flex-row flex-col items-center justify-between gap-6'>
                    ...
                    <div className='w-full md:w-4/5'>
                        ...
                        <p className='hidden mdl:inline-flex pr-10 text-sm'>{item.description.substring(0,200)}</p>
                        <p className='text-sm md:text-base'>
                            Unit Price
                            <span className='font-semibold'>${item.price}</span>
                        </p>
                        <div className='bg-[#F0F2F2] flex justify-center items-center gap-1 w-24 py-1 text-center 
                                        drop-shadow-lg rounded-md text-xs md:text-sm'>
                          <p>Qty:</p>                
                          <p onClick={()=>dispatch(decrementQuantity(item.id))}
                           className='cursor-pointer bg-gray-200 px-1 rounded-md hover:bg-gray-400 
                                        duration-300'>-</p>
                          <p>{item.quantity}</p>
                        ...
                        <button onClick={()=>dispatch(deleteItem(item.id))}
                         className='bg-red-500 text-sm w-35 p-2 rounded-lg text-white mt-2 hover:bg-red-700 
                                           active:bg-red-900 duration-300'>
                          Delete Item
                        </button>
                    </div>
                    <div>
                      <p className='text-sm md:text-lg font-titleFont font-semibold'>
            ...
            <div className='w-full py-2'>
              <button onClick={()=>dispatch(resetCart())}
               className='flex mx-auto mdl:mx-3 px-10 py-2 bg-red-500 hover:bg-red-600 active:bg-red-500 text-white 
                 rounded-lg font-titleFont font-semibold text-lg tracking-wide'>
                Clear Cart
              </button>
            </div>
        </div>
        <div className='w-full h-52 bg-white col-span-4 xl:col-span-1 flex flex-col justify-center items-center p-4 '>
          <div>
          ...
          <button className='w-full font-titleFont font-medium text-base bg-gradient-to-tr from-yellow-400 
            to-yellow-200 border hover:from-yellow-300 hover:to-yellow-400 border-yellow-500 
          ...
          <div className='w-96 p-4 bg-white flex flex-col items-center rounded-md shadow-lg'>
            ...
            <p className='text-sm text-center'>
               Your shopping lives to serve. Give it purpose - fill it with books, electronics, videos, etc.
               and make it happy.
            </p>
            <Link to = '/'>
