         ...
         <Link to = '/'>
           <div className='headerHover'>
            <img className='w-24 mt-2'src={logo} alt=''/>
                          ...
                          {/*DELIVER & LOCATION SECTION*/}
          <div className='headerHover hidden mdl:inline-flex'>
            <LocationOnOutlinedIcon/>
            <p className='text-sm text-lightText font-light flex flex-col'>Deliver to{" "}
            <span className='text-sm font-semibold -mt-1 text-whiteText'>Dubai</span></p>
          </div>

                       {/*DROPDOWN & SEARCHBAR SECTION*/}
          <div className='h-10 rounded-md hidden lgl:flex flex-grow relative'>
           <span onClick={()=>setShowAll(!showAll)}
            className='w-14 h-full text-amazon_blue text-sm text-titleFont bg-gray-200 hover:bg-gray-300
                       cursor-pointer duration-300 flex items-center justify-center rounded-tl-md
                      rounded-bl-md '>
              ...
              showAll && (
                   <div>
                    <ul className='absolute w-56 h-80 top-10 left-0 overflow-y-scroll overflow-x-hidden bg-white
                                   border-[1px] border-amazon_blue text-black p-2 flex-col gap-1 z-50 '> 
                         {
                          allItems.map((item)=>(
            ...
            type='text'/>
            <span className='w-12 flex h-full items-center justify-center bg-amazon_yellow hover:bg-[#f3a847]
         ...
         <div className='flex flex-col items-start justify-center headerHover'>
          {
            ...
            <p className='text-sm font-semibold text-whiteText -mt-1 hidden mdl:inline-flex'>Accounts & Lists{" "}
          ...
          </Link>
          {
            userInfo && (
              <div onClick={handleLogOut}
                ...
                <LogoutIcon/>
                <p className='hidden mdl:inline-flex text-xs font-bold text-whiteText'>
                  Log out
                </p>
              </div>
            )
